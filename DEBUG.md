...
## Môi trường smoke test — ưu tiên SPX tab trong --app window

`vit.bat` mở Edge với `--app=http://localhost:9009/` → tạo **KiotVit app window** (standalone, không address bar). Flag `--remote-debugging-port=9222` áp dụng cho toàn bộ Edge process, kể cả các tab SPX người dùng mở thêm bên trong window đó.

Trong thực tế, user mở `sp.spx.shopee.vn` như một tab bên trong KiotVit app window. `list_pages` vì vậy sẽ show cả hai:
- `http://localhost:9009/` — KiotVit (không liên quan đến SPX scripts)
- `https://sp.spx.shopee.vn/...` — **tab SPX cần smoke test**

**Ưu tiên tab `sp.spx.shopee.vn` khi smoke test** — userscripts của SPX Helpers chạy ở đây, đúng profile, đúng Tampermonkey instance.

## SPX Service Point — Installed PWA

SPX Service Point cũng là một **installed PWA** trong Edge (app riêng, có thể mở độc lập):
...
Tuy nhiên trong workflow hàng ngày, SPX được dùng như tab bên trong KiotVit window (không phải PWA window riêng), nên `display-mode: standalone` = `false` là bình thường. Userscripts vẫn chạy đầy đủ.
