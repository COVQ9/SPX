...
## Môi trường smoke test — ưu tiên --app window

Port 9222 thuộc về Edge window được `vit` mở với flag `--app=http://localhost:9009/`. Đây là **standalone app window** (không có address bar, chạy tách biệt). Tất cả tab SPX người dùng mở trong giờ làm việc đều nằm trong window này.

**Ưu tiên smoke test trong window này** — đây là môi trường thật, đúng profile, đúng Tampermonkey instance đang chạy userscripts. Không smoke test trên Edge window mở thường (khác profile, không có Tampermonkey với scripts đang deploy).
