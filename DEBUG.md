...
| App | Taskbar | URL | Display mode |
|-----|---------|-----|--------------|
| **SPX Service Point** | icon thứ 2 (X đỏ) | `https://sp.spx.shopee.vn/` | `minimal-ui` |
| **KiotVit** | icon thứ 5 (vàng) | `http://localhost:9009/` | standalone (`--app`) |
...
**Ưu tiên smoke test trong SPX Service Point window** — userscripts chạy ở đây. Nhận biết: `list_pages` show URL `https://sp.spx.shopee.vn/...`.
...
- **Display mode:** `minimal-ui` (có thanh điều hướng tối giản, không phải full address bar)
